...
func (s *store) save() {
	...
	if err != nil {
		log.Println("err open store bak file %v", err)
		return
	}
	defer bk.Close()
	...
	if err != nil {
		log.Println("err open store file %v", err)
		return
	}
	defer dst.Close()
	...
	if err != nil {
		log.Println("err copy store file %v", err)
		return
	}
...
func (s *store) load() {
	...
	if err != nil {
		log.Println("err load store file %v maybe first start,please ignore", err)
		return
	}
	defer fReader.Close()
